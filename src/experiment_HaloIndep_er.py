...
#        xtable_list = par.parmap(self.TabulateMaximumGapLimit, kwargs, processes)
        xtable_list = np.array([self.TabulateMaximumGapLimit(**k) for k in kwargs])
        for v_index in range(vmin_list.size):
...
#            self.BinSize
        self.Expected_limit = module.Expected_limit * self.BinSize  # counts/day/kg
